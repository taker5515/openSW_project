...
def _build_chart_data(
    ...
    """Build chart_data dict from normalized statement lists."""
    revenue_chart = [
        {"period": d["period"], "value": d.get("revenue")} for d in income_data
    ]
    net_income_chart = [
        {"period": d["period"], "value": d.get("net_income")} for d in income_data
    ]
    operating_income_chart = [
        {"period": d["period"], "value": d.get("operating_income")} for d in income_data
    ]
    free_cf_chart = [
        {"period": d["period"], "value": d.get("free_cash_flow")} for d in cashflow_data
    ]
    margins_chart = [
        {
            "period": r["period"],
    ...
    debt_ratio_chart = [
        {"period": r["period"], "value": r.get("debt_ratio")} for r in ratios_data
    ]

    return {
        "revenue": revenue_chart,
        "net_income": net_income_chart,
        "free_cash_flow": free_cf_chart,
        "operating_income": operating_income_chart,
        "margins": margins_chart,
        "debt_ratio": debt_ratio_chart,
    }
...
def get_income_statement(
    ...
    try:
        ...
        if df is None or df.empty:
            raise HTTPException(
                status_code=404,
                detail=f"No financial data found for {symbol}",
            )
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Failed to retrieve income statement for {symbol}",
        )
...
def get_balance_sheet(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Failed to retrieve balance sheet for {symbol}",
        )
...
def get_cash_flow(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Failed to retrieve cash flow for {symbol}",
        )
...
def get_ratios(
    ...
    try:
        ...
        ratios_data = financial_ratio_service.calculate_ratios(
            income_data, balance_data, cashflow_data, period
        )
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Failed to retrieve ratios for {symbol}",
        )
...
def get_segments(
    ...
    symbol = symbol.upper()
    try:
        result = financial_segment_service.get_segments(symbol, period)
        return result
    except Exception:
        raise HTTPException(
            status_code=404,
            detail=f"Failed to retrieve segment data for {symbol}",
        )
...
def get_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404,
            detail=f"Failed to retrieve summary for {symbol}",
        )
